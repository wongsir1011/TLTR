...
export async function summarizeForPartner(
  ...
  const prompt = `你是一個超級貼心的溝通大師。
現在有一篇長文/文章內容，我需要你將它精簡為正好「三個重點」。
...
`;
  ...
  try {
    ...
    const result = JSON.parse(response.text || '{"points": []}') as SummaryPoints;
    return result.points;
  } catch (error) {
    console.error("Gemini Error:", error);
    throw new Error("無法生成摘要，請稍後再試。");
  }
}
