...
async function startServer() {
  ...
  if (!geminiKey || geminiKey === "MY_GEMINI_API_KEY") {
    console.error("WARNING: GEMINI_API_KEY is not set or is a placeholder.");
  }
  ...
  app.post("/api/summarize", async (req, res) => {
    const { content, personality, receptivity, outputLanguage } = req.body;
    ...
    try {
      ...
      if (!apiKey || apiKey === "MY_GEMINI_API_KEY") {
        console.error("GEMINI_API_KEY is missing or set to placeholder.");
        return res.status(500).json({ 
          error: "API 金鑰未設置。請在 AI Studio 的 Settings -> Secrets 中添加名為 GEMINI_API_KEY 的金鑰。" 
        });
      }
